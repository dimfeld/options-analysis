...
function analyzeSide(config, allExpirations) {
    ...
    let result = _.map(expirations, (expiration) => {
        ...
        return Object.assign({ deltas }, expiration);
    });
    return result;
}
...
function analyzeLiquidity(config, chain) {
    ...
    let results = _.flatMap(allData, (expiration) => {
        return _.chain(expiration.deltas)
            ...
            return Object.assign({ expiration: expiration.expiration, targetDte: expiration.target, targetDelta: delta.target, spreadPercent: contract.bid ? ((contract.ask / contract.bid) - 1) * 100 : 1000 }, _.pick(contract, ['symbol', 'delta', 'putCall', 'strikePrice', 'daysToExpiration', 'bid', 'ask', 'totalVolume', 'openInterest']));
        })
