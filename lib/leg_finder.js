...
function closestDeltas(strikes, deltas) {
    ...
    let closest = _.map(deltas, (targetDelta) => {
        ...
        let greaterDistance = index < sorted.length
...
function analyzeLiquidity(config, chain) {
    ...
    let results = _.flatMap(allData, (expiration) => {
        return _.chain(expiration.deltas)
            ...
            return Object.assign({ expiration: expiration.expiration, targetDte: expiration.target, targetDelta: delta.target, spreadPercent: contract.bid
