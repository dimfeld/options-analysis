...
export function closestDeltas(strikes: StrikeMap, deltas: number[]) {
  ...
  let closest = _.map(deltas, (targetDelta) => {
    ...
    let index = _.sortedIndexBy(sorted, { delta: targetDelta }, (x) => Math.abs(x.delta));
...
export function closestAfterDte(dates: ExpirationDateMap, dteTarget: string[]): ClosestDte[] {
  let closestDte = _.map(dteTarget, (target) => {
    let dteNum = Number.parseInt(target, 10);
    let requireMonthly = target[target.length - 1] === 'M';
    return { target: dteNum, dte: null, expiration: null, difference: Infinity, strikes: null, requireMonthly };
  });
...
export function analyzeSide(config: AnalyzeSideOptions, allExpirations: ExpirationDateMap) {
...
export function filterLiquidity(config: FilterLiquidityArguments, data : LiquidityInfo) {
...
export function analyzeLiquidity(config: AnalyzeSideOptions&FilterLiquidityArguments, chain: AnalyzeLiquidityOptions) {
